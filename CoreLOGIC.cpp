...
//--------------------------------------------------------------------------------------------------------------------------------------
// это событие вызывается для обработки критических ко времени выполнения участков кода. Здесь, например, можно обновлять энкодер.
//--------------------------------------------------------------------------------------------------------------------------------------
void CoreLogic::onCritical()
{
...
/*  
  ...
  SIM800.sendSMS(phoneNumber, F("Не звони мне больше, редиска :)"), true); // последний параметр - flash или обычное смс
*/  
#endif  
}
//--------------------------------------------------------------------------------------------------------------------------------------
...
// Обращаю внимание, что ядро стартует каждый раз при перезаливке конфига из конфигуратора,
// при этом событие onCoreStarted вызывается каждый раз, в отличие от doBegin, которая вызывается один раз в начале работы.
// Поэтому событие onCoreStarted является единственной правильной точкой для добавления своих динамических датчиков в систему.
//--------------------------------------------------------------------------------------------------------------------------------------
